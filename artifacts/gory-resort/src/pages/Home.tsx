...
import { listingCoverImage } from '@/lib/listingIntegrity';
...
const FEATURED_IDS = [12, 20, 18, 19] as const;
const FEATURED_PROPERTIES = FEATURED_IDS.map((id) => {
  const l = listingById(id);
  if (!l) return null;
  const countryLabel =
    l.country === 'RS' ? ', Сербия' : l.country === 'ME' ? ', Черногория' : '';
  return {
    id: l.id,
    image: listingCoverImage(l),
    location: `${l.city}${countryLabel}`,
    type: l.type,
    price: l.price,
    district: l.district,
    beds: l.beds,
  ...
  return (
    <Layout>
      ...
      <section ref={heroSectionRef} className="relative dark:bg-black bg-white overflow-hidden">
        ...
        {/* Top padding is modest: Layout main already clears the fixed nav. */}
        <div className="container mx-auto px-6 pt-8 md:pt-12 pb-12 lg:pb-0 relative z-10">
