    ...
    <Layout>
      ...
      <section className="relative pt-28 md:pt-36 pb-12 md:pb-24 overflow-hidden dark:border-b dark:border-white/5 border-b border-black/5">
        ...
        <div className="container mx-auto px-6 relative z-10 flex flex-col md:flex-row items-center justify-between gap-8 md:gap-12">
          ...
          <div className="w-full md:w-1/2 flex flex-col relative z-20">
            ...
            <nav className="flex flex-wrap items-center gap-x-3 gap-y-1 text-[12px] md:text-[13px] font-space-grotesk dark:text-white/40 text-foreground/50 mb-6 md:mb-8" aria-label="breadcrumb">
              <Link href="/" className="dark:hover:text-white/80 hover:text-foreground/80 transition-colors flex items-center min-h-[48px]">Главная</Link>
              <span className="dark:text-white/20 text-foreground/25">/</span>
              <Link href="/properties" className="dark:hover:text-white/80 hover:text-foreground/80 transition-colors flex items-center min-h-[48px]">Объекты</Link>
              <span className="dark:text-white/20 text-foreground/25">/</span>
              <span className="dark:text-white/80 text-foreground/80 flex items-center min-h-[48px]">{data.nameRu}</span>
            </nav>

            <h1 className="font-oxanium text-[clamp(2.75rem,7vw,4.5rem)] font-medium dark:text-white text-foreground leading-[1.0] tracking-[-0.03em] chrome-text drop-shadow-md mb-6">
              {data.nameRu}
            </h1>

            <p className="font-space-grotesk dark:text-white/60 text-foreground/65 text-lg md:text-xl max-w-xl leading-relaxed tracking-[-0.01em] mb-10">
              {data.tagline}
            </p>

            {/* Quick stats row */}
            <div className="flex flex-wrap items-end gap-x-8 gap-y-8 pt-8 dark:border-t dark:border-white/[0.08] border-t border-black/[0.06] w-full">
              {data.stats.map((s, i) => (
                <div key={i} className="flex flex-col">
                  <span className="font-oxanium text-[clamp(1.5rem,3vw,2rem)] dark:text-white text-foreground font-medium leading-none mb-2 tracking-tight drop-shadow-sm">{s.value}</span>
                  <span className="font-space-grotesk text-[10px] md:text-[11px] dark:text-white/40 text-foreground/50 uppercase tracking-[0.08em]">{s.label}</span>
                </div>
              ))}
              <div className="ml-auto flex flex-col text-right mt-2 sm:mt-0">
                ...
                <span className="font-space-grotesk text-[10px] dark:text-white/30 text-foreground/40 mt-2 uppercase tracking-[0.08em]">Минимальный вход</span>
              </div>
            </div>
          </div>
      ...
      <section className="dark:border-b dark:border-white/5 border-b border-black/5 dark:bg-[#030303] bg-[#F5F3EE] py-4 md:py-5 relative z-20">
        <div className="container mx-auto px-6 flex flex-wrap items-center justify-center gap-x-6 gap-y-3 text-[11px] md:text-[13px] font-space-grotesk dark:text-white/40 text-foreground/50">
          ...
          <div className="hidden md:block w-px h-4 dark:bg-white/10 bg-black/10" />
          <div className="flex items-center gap-2">
            <Building2 className="w-4 h-4 dark:text-white/30 text-foreground/40" />
            <span className="tracking-wide uppercase md:normal-case">7 лет на рынке</span>
          </div>
          <div className="hidden md:block w-px h-4 dark:bg-white/10 bg-black/10" />
          <div className="flex items-center gap-2">
            <Scale className="w-4 h-4 dark:text-white/30 text-foreground/40" />
      ...
      {/* ─── WHY THIS COUNTRY ──────────────────────────────────────────────── */}
      <section className="py-20 md:py-32 dark:bg-[#050505] bg-white">
        <div className="container mx-auto px-6">
          <div className="flex items-center gap-4 mb-12 md:mb-16">
            <h2 className="font-oxanium text-[11px] md:text-[12px] tracking-[0.2em] dark:text-white/30 text-foreground/40 uppercase">
              Почему {data.nameRu}
            </h2>
            <div className="h-px dark:bg-white/10 bg-black/10 flex-1 max-w-[120px]"></div>
          </div>
          ...
          <div className="grid md:grid-cols-3 gap-6 lg:gap-8">
            {data.features.map((f, i) => (
              ...
              >
                ...
                <div>
                  <h3 className="font-oxanium dark:text-white/90 text-foreground font-medium text-[20px] leading-snug mb-3 tracking-[-0.01em]">{f.title}</h3>
                  <p className="font-space-grotesk dark:text-white/50 text-foreground/60 text-[14px] leading-relaxed tracking-wide">{f.body}</p>
                </div>
              </div>
            ))}
          </div>
      ...
      {/* ─── MARKETS TABLE ─────────────────────────────────────────────────── */}
      <section className="py-20 md:py-32 dark:bg-[#020202] bg-[#F5F3EE] dark:border-y dark:border-white/5 border-y border-black/5">
        <div className="container mx-auto px-6 max-w-5xl">
          <div className="mb-12 md:mb-16 text-center md:text-left">
            <h2 className="font-oxanium text-[11px] md:text-[12px] tracking-[0.2em] dark:text-white/30 text-foreground/40 uppercase mb-4">
              Рынки и ликвидность
            </h2>
            <p className="font-oxanium text-[28px] md:text-[36px] dark:text-white text-foreground font-medium tracking-tight">
              Локации и порог входа
            </p>
          </div>

          {/* Editorial, high-craft list. No generic table borders, just clean typography and spacing. */}
          <div className="dark:border dark:border-white/[0.06] border border-black/[0.06] rounded-xl dark:bg-[#060606] bg-white shadow-2xl flex flex-col">
            {data.markets.map((m, i) => (
              ...
              >
                ...
                <div className="md:w-1/2 min-w-0">
                  <span className="font-space-grotesk dark:text-white/50 text-foreground/60 text-[14px] leading-relaxed block">{m.highlight}</span>
                </div>
      ...
      <section className="py-20 md:py-32 dark:bg-[#050505] bg-white">
        <div className="container mx-auto px-6 max-w-5xl">
          <div className="flex items-center justify-center md:justify-start gap-4 mb-12 md:mb-16">
            <Landmark className="w-5 h-5 dark:text-white/30 text-foreground/40" />
            <h2 className="font-oxanium text-[11px] md:text-[12px] tracking-[0.2em] dark:text-white/30 text-foreground/40 uppercase text-center md:text-left">
              Юридическая и налоговая база (2026)
            </h2>
          </div>
          
          <div className="grid md:grid-cols-2 gap-6 lg:gap-8">
            <div className="dark:border dark:border-white/[0.05] border border-black/[0.06] rounded-xl p-8 dark:bg-[#080808] bg-[#F5F3EE] relative overflow-hidden group dark:hover:border-white/[0.12] hover:border-black/10 transition-colors">
              <div className="absolute top-0 right-0 p-8 opacity-[0.03] group-hover:opacity-[0.06] transition-opacity">
                <Scale className="w-32 h-32" />
              </div>
              <div className="relative z-10">
                <p className="font-oxanium text-[10px] md:text-[11px] tracking-[0.2em] dark:text-white/40 text-foreground/50 uppercase mb-3">
                  Резиденция и Паспорт
                </p>
                <h3 className="font-oxanium text-[22px] dark:text-white text-foreground font-medium tracking-tight mb-6">Виза и ВНЖ</h3>
                <p className="font-space-grotesk dark:text-white/60 text-foreground/65 text-[14px] leading-relaxed tracking-wide">{data.visa}</p>
              </div>
            </div>
            ...
            <div className="dark:border dark:border-white/[0.05] border border-black/[0.06] rounded-xl p-8 dark:bg-[#080808] bg-[#F5F3EE] relative overflow-hidden group dark:hover:border-white/[0.12] hover:border-black/10 transition-colors">
              ...
              <div className="relative z-10">
                ...
                <h3 className="font-oxanium text-[22px] dark:text-white text-foreground font-medium tracking-tight mb-6">Налогообложение</h3>
                <p className="font-space-grotesk dark:text-white/60 text-foreground/65 text-[14px] leading-relaxed tracking-wide">{data.taxNote}</p>
              </div>
            </div>
          </div>
        </div>
      ...
      {/* ─── CTA STRIP ─────────────────────────────────────────────────────── */}
      <section className="dark:bg-[#080808] bg-[#F5F3EE] dark:border-t dark:border-white/10 border-t border-black/10 relative overflow-hidden">
        ...
        <div className="container mx-auto px-6 py-16 md:py-24 flex flex-col lg:flex-row items-center justify-between gap-10 relative z-10 text-center lg:text-left">
          <div className="max-w-2xl">
            <h3 className="font-oxanium text-[24px] md:text-[32px] font-medium dark:text-white text-foreground mb-4 tracking-[-0.01em] leading-snug">
              Приватная консультация
            </h3>
            <p className="font-space-grotesk text-[15px] dark:text-white/50 text-foreground/60 leading-relaxed max-w-xl mx-auto lg:mx-0">
          ...
          <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-4 w-full lg:w-auto">
            <a
              ...
              className="flex items-center justify-center gap-3 min-h-[52px] px-8 rounded-full dark:border dark:border-white/15 border border-black/10 dark:text-white/80 text-foreground/70 dark:hover:text-white hover:text-foreground dark:hover:border-white/40 hover:border-black/20 dark:hover:bg-[#141414] hover:bg-white transition-all dark:bg-[#0c0c0c] bg-white/60 w-full sm:w-auto group"
            >
              <MessageCircle className="w-5 h-5 dark:text-white/40 text-foreground/50 group-hover:text-green-500/90 transition-colors" />
